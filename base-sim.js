...
function saveSettings() {
  settings = sanitizeUiSettings(settings);
...
function createDefaultTimelineSeries() {
  return { cumulative: true, morale: true, parameters: true };
}

function sanitizeTimelineSeries(value = {}) {
...
function formatIntermediateValue(value) {
...
function formatMatchedSkillText(skill) {
...
function formatShiftOperatorSkillSummary(skill) {
...
function formatShiftOperatorMoraleText(morale) {
...
function formatMoraleChange(value) {
  const number = Number(value || 0);
  const sign = number > 0 ? "+" : "";
...
function formatMaxCondition(conditions) {
